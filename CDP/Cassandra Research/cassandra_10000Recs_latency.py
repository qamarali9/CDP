...
t2 = datetime.now()
print("The latency for storing 10000 records : {}".format(t2-t1))


t1 = datetime.now()
# Printing all rows
rows = session.execute("SELECT * FROM test_table;")
for row in rows:#rows[0:100]:
    print(row)
t2 = datetime.now()
print("The latency for fetching and printing 10000(all) records : {}".format(t2-t1))
...
for row in rows[0:100]:
    print(row)
t2 = datetime.now()
print("The latency for fetching a single record : {}".format(t2-t1))
